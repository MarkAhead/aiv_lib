...
GEMINI_API_KEY = get_config_value("gemini_key")
...
def get_response_from_model(prompt_text: str, model: str = GPT_4):
    response = client.chat.completions.create(
        model= model,
        messages=[
            {"role": "system", "content": "You are a helpful assistant."},
...
def return_empty_social_media_posts(video_post_data_list):
    return [
        (key, data) for key, data in video_post_data_list 
        if data.get('document_state') 
        and "PROMPT_CREATED" in data.get('document_state')
        and "INITIAL_ARTIFACTS_CREATED" not in data.get('document_state')
    ]
...
def call_openai_using_instructor(prompt, response_model,  model_name="gpt-4o"):
